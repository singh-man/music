...
//        return new File(f.getAbsolutePath().replace(ext, newExt));
    ...
    static AVData volume(AVData data) {
        String db = input("enter db to raise");
        String out = replaceExtension(data.getFile(), "_v" + db + ".mkv");
        String command = String.format("%s -i %s -map 0 -c copy -c:a aac -af \"volume=%sdB\" %s", "ffmpeg", IAV.addDoubleQuotes(data.getFile()),
    ...
    static AVData encode(AVData data) {
        ...
        String out = replaceExtension(data.getFile(), "_" + videoEncoders.get(Integer.parseInt(encoder)) + ".mkv");
        ...
        String command = String.format("%s -i %s -vf scale=%s -map 0 -c copy -c:v %s -preset medium %s -crf %s %s",
                "ffmpeg", IAV.addDoubleQuotes(data.getFile()), videoResolution.get(Integer.parseInt(resolution)),
